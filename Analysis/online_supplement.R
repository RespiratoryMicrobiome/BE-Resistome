...
pacman::p_load(pacman, ggplot2, tidyverse, tidyr, wesanderson, reticulate, vegan, dplyr, reshape2, RColorBrewer, phyloseq, SNFtool)
...
qPCR_plot_PA_erd <-ggplot(long_qPCR_2[which(long_qPCR_2$Copies == "PA_qPCR"),], aes(x = Time.point, y = Value_log)) +
  ...
  labs(x = "", y = expression(paste("log10 (16S P.aeruginosa gyrB gene copies /", mu, "l)"))) +  scale_x_discrete(labels = c("Pre", "Post"))+
